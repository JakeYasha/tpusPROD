...
<div class="jcarousel-wrapper">
	<div class="pop_item_slider jcarousel js-jcarousel">
		...
		<?$i=0;foreach ($items as $item) {$i++;?>
			<li class="leaf">
				<div class="image"><a<?if($item['flag_is_referral']){?> rel="nofollow"<?}?> href="<?=$item['link']?>"><?if($item['image_thumb']){?><img src="<?=$item['image_thumb']?>" alt="<?=$item['name']?>" /><?} elseif($item['image']){?><img src="<?=$item['image']?>" alt="<?=encode($item['name'])?>" /><?} else {?><img src="/css/img/no_img.png" alt="Нет фотографии" /><?}?></a></div>
