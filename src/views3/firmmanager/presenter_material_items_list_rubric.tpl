    ...
    <table class="default-table rubric-table" style="width: 100%;">
        ...
        <? foreach ($items as $item) { ?>
            <tr<? if (!$item->val('flag_is_active')) { ?> style="opacity: .5"<? } ?>>
                ...
                <td style="text-align:center;">
                <button onclick="javascript:void();" class="material-button material-publish-button" title="Добавить" alt="Добавить" data-id="<?= $item->id() ?>" data-preview_link="<?= $item->val('preview_link') ?>" data-mnemonic="<?= $item->val('mnemonic') ?>"></button>
                </td>
            </tr>
    <? } ?>
    </table>
