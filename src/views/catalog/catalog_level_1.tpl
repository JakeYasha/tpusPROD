...
<?if($rubrics !== '') {?>
<div class="red-headline">Популярные категории каталога</div>
<?=$rubrics?>
<?}?>
<?=$promo?>
<div class="clearfix">
...
<? if ($items && $childs && $mode === 'goods') {?>
		...
		<?foreach ($items as $arr) {?>
			...
			<div class="headline"><a href="<?= app()->link('/catalog/'.$arr['id_group'].'/'.($mode === 'goods' ? '' : $arr['id_subgroup'].'/'))?>"><?=$arr['name']?></a></div>
