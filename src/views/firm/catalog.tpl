...
<?if($rubrics !== '') {?>
	<div class="red-headline">Популярные категории</div>
	<?=$rubrics?>
<?}?>
<div class="cat_description">
	<?=str()->replace(app()->config()->get('app.firm.catalog', ''), ['_Cp_', '_Cg_', '_L_'], [app()->location()->currentName('prepositional'), app()->location()->currentName('genitive'), app()->location()->currentId()])?>
</div>
<?/*$tabs*/?>
<div class="tags_field js-no-height firm-catalog-top">
	<ul>
		<li><a href="<?=app()->link('/firm/new/')?>">Новые фирмы</a></li>
		<li><a href="<?=app()->link('/firm/popular/')?>">Популярные фирмы</a></li>
		<li><a href="<?=app()->link('/firm/best/')?>">Лидеры по рейтингу</a></li>
	</ul>
</div>
<? if ($mode === 'catalog') {?>
	<div class="clearfix">
		<div class="clearfix firm-catalog">
			<div class="black-block">Полный рубрикатор каталога фирм</div>
			<? if ($items) {?>
			<?$j = 0;$half = ceil(count($items) / 2);
			foreach ($items as $mtKey => $arr) {$j++;$mtKey = $arr['mtkey'];?>
			<div class="headline"><a href="<?= app()->link('/firm/bytype/' . $mtKey . '/')?>"><?= $main_types[$mtKey]['name']?></a></div>
			<div class="tags_field">
				<ul>
					<?/*@var $item Catalog*/?>
