	...
	<div class="tags_field">
		<ul>
			...
			foreach ($items as $item) {
				...
				?>
			<li><a href="<?= (isset($link) && $link) ? app()->linkFilter($link, array_merge($_filter, ['id_catalog' => $item->id()])) : app()->link(app()->linkFilter($item->link(), $_filter))?>" rel="nofollow"><?= $item->name('tags')?></a></li>
		<? }?>
		</ul>
	<? if (count($items) > 6) {?>
			<div class="show_more">
