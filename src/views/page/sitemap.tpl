	...
	<div class="for_clients_text_c clearfix page">
		<div class="for_clients_list page-sitemap">
			...
			<ul>
				...
				<?$show = false;foreach ($id_cities as $id_city) {if($counts[$id_city]['firms'] >= 100){$show=true;}}?>
				<?if($show){?>
					<li><span><?=$regions[$id_region]?></span> (<a href="<?=APP_URL?>/ratiss/service/<?=$services[$id_region]['id_service']?>/">региональный представитель</a>)</li>
					<?foreach ($id_cities as $id_city) {if(!$cities[$id_city])continue;if($counts[$id_city]['firms'] < 100)  continue;
			...
			<br/>
			<ul>
				<li><a href="<?=APP_URL?>/photo-contest/">Фотоконкурсы на сайте</a></li>
				<li><a href="<?=APP_URL?>/consumer/">Вопросы и ответы по защите прав потребителей</a></li>
			</ul>
		</div>
	</div>
</div>