<? if ($items) {
?>
<table class="default-table rubric-table rubric-table-use" style="width: 100%;">
        <tr>
        ...
        <? foreach ($items as $item) { ?>
            <tr<? if (!$item->val('flag_is_active')) { ?> style="opacity: .5"<? } ?> id="item-use-<?=$item->id();?>">
                ...
                <button onclick="ajax_material(this, <?= $item->id() ?>,'del');" class="material-button" title="Добавить в выпуск" alt="Добавить в выпуск" data-id="<?= $item->id() ?>" data-preview_link="<?= $item->val('preview_link') ?>" data-mnemonic="<?= $item->val('mnemonic') ?>" style="width: 100%;">Удалить из выпуска</button>
