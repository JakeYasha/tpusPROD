...
<? if ($items) { ?>
    <table class="default-table rubric-table rubric-table-all" style="width: 100%;">
        <tr>
        ...
        <? foreach ($items as $item) { ?>
            <tr<? if (!$item->val('flag_is_active')) { ?> style="opacity: .5"<? } ?>>
                ...
                <td style="text-align:center;">
                <button onclick="ajax_material(this, <?= $item->id() ?>);" class="material-button" title="Добавить в выпуск" alt="Добавить в выпуск" data-id="<?= $item->id() ?>" data-preview_link="<?= $item->val('preview_link') ?>" data-mnemonic="<?= $item->val('mnemonic') ?>" style="width: 100%;">Добавить в выпуск</button>
                </td>
            </tr>
    <? } ?>
    </table>
