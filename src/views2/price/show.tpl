...
<div class="firm_field single_item">
	...
	<div class="description">
        ...
        <? if (str()->length($item['vendor']) > 2) {?><p>Производитель: <strong><?=$item['vendor']?></strong></p><?}?>
		...
		<div class="price_contact_field">
			...
			<div class="contacts">
				...
				<p><?= $firm->address()?></p>
				<? if ($firm->hasWeb()) {?>
					<p><span class="r"><?= $firm->renderWebLinks()?></span>
					</p>
				<? }?>
			</div>
		</div>
...
<div class="item_info">
	<div class="search-result description">
		...
		<?if(strlen($firm_catalog_name_analog_prices) > 2) {?>
			<a href="<?=$current_catalog_link?>"><div class="red-block">Посмотрите обязательно другие фирмы предлагающие "<?=$firm_catalog_name_analog_prices?>" >>></div></a>
		<?}?>
	</div>
	<?= $price_on_map?>
