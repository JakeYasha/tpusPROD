...
class AdvertModuleFullImage extends \App\Action\FirmUser\Ajax\Upload {
	...
	public function execute() {
		...
		if ($this->fileUploader()->uploadFile()) {
			...
			if ($image->embeddedFile()->isImage()) {
				...
				if ($image->val('file_extension') !== 'gif' && (int) $image->val('file_dimension_width') > 500) {
					$this->imageProcessor()->setSourceFilePath($image->embeddedFile()->path())
							->setTargetFilePath($image->embeddedFile()->path())
							->setTargetFileWidth(500)
				...
				$result = [
					...
					'image_type' => 'advert-module-full-image'
				];
			}
		}
