...
use App\Classes\SmsServiceApi;

class SmsSender extends \App\Action\Crontab {

    public static $email_addresses = [
        'mng@727373.ru',
        'zapros@727373.ru',
    ...
    private function send() {
        ...
        $hostname = '{smtp.yandex.ru:465/ssl}SMTP';
        //E-mail:
        $username = 'podatiyar@yandex.ru';
        
        $password = 'RUtaroAB6u6)';//общий
        $password = 'azlmkhaduehgrwtm';//приложения
        $last_message = 10;

        $mbox = imap_open($hostname, $username, $password) or die('Cannot connect to SMTP: ' . imap_last_error());
        $mc = imap_check($mbox);
...
//                var_dump($result);
    ...
    public static function prepareText($s, $addr) {
        ...
        if ($addr == self::$email_addresses[0])
            $s = 'Ответ на запрос от 727373 ' . $s;
        $ss = explode("\n", $s);
        if (mb_strlen($ss[0]) > 70 * 3)
            $itog = mb_substr($ss[0], 0, 70 * 3);
        else
            $itog = $ss[0];
