...
class Statistics727373 extends FirmManager {

	public function execute($html_mode = false) {
		app()->breadCrumbs()->setElem('Запросы 727373', self::link('/statistics727373/'));
		...
		$this->params = app()->request()->processGetParams([
            ...
            'export' => ['type' => 'string'],
            'group' => ['type' => 'string', 'default_val' => 'months'],
		]);
		...
		$this->text()->getByLink('firm-manager/statistics727373');
		app()->metadata()->setFromModel($this->text());
		if (!$this->text()->exists()) {
			app()->metadata()->setTitle('Запросы 727373');
		}
        ...
        if ($this->params['id_firm']) {
            if ($this->params['export'] === 'xls') {
                $presenter->find727373Pages($this->params);
            } else {
                $presenter->findStatistics727373($this->params);
            }
        }
    ...
    private function exportStatistics($data) {
        ...
        $title = 'Отчет о запросах с сайта 727373.ru';
        $firm_name = '';
        $firm_title = '';
        $type = 'Запросы с сайта 727373.ru';
        $total_count = 0;
